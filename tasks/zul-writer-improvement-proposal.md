...
**B2. ZK 9 support is claimed but not wired through. ✅ FIXED (2026-07-07).**
...
*Fix applied:* (1) Step 3 now documents `--zk-version <detected>` in both the primary and fallback commands. (2) Layer 4 is now version-aware and **always runs** (renamed to "Version Compatibility", label shows the target major, e.g. `Layer 4: ZK 9 Compatibility`). `parse_major_version()` extracts the leading integer (`"9 or before"`→9, `"10.3.0"`→10). Removed *attributes* (all dropped by ZK 8.x) are flagged for any target; the removed *component* `<fragment>` (10.2.0) is now gated to targets ≥10, so **ZK 9 no longer gets the false `<fragment>`-removed warning** — the core complaint. (3) The reverse blocklist `NEW_IN_ZK10_ATTRIBUTES` was populated with attributes **verified against the ZK component reference** to be ZK-10-only — `accept` on `<dropupload>` (10.0.0) and `responsive`/`responsiveColumns`/`responsiveVisible` on `<grid>`/`<column>` (10.4.0). It's component-gated (avoids false positives on same-named attributes) and fires only for pre-10 targets. Note: ZK 10.0.0 introduced **no new standard ZUL component tags** (it was architectural — stateless components, client MVVM, TypeScript, Font Awesome 6), so the blocklist is attribute-only by design rather than fabricating nonexistent tags. Verified: default (v10) runs classify identically (regression suite: 0 regressions); a `<fragment>` with `accept`/`responsive` correctly passes the fragment on `--zk-version 9` while flagging the two attributes, and flags `<fragment>` (not the attributes) on `--zk-version 10.3.0`.

**B3. Fragment validation. ✅ FIXED (2026-07-07).**
...
*Fix applied:* dogfooding showed the real false-failure class is **multi-root fragments** (several top-level components), which fail Layer 1 with "junk after document element" — Layer 2 never even runs. Added `wrap_fragment_in_zk()`: when Layer 1 fails, the validator wraps the body in a single namespaced `<zk>` root and retries; if the wrapped form is well-formed it proceeds (all downstream layers use the wrapped copy) and prints `✓ PASS (fragment wrapped in <zk> for validation)`. The `<zk>` open tag is inserted with no trailing newline so **body line numbers are preserved** in Layer 2–4 error messages. This is a fallback (only triggers on Layer 1 failure), so single-root files are untouched — zero corpus churn. Genuinely malformed files (e.g. an unclosed tag) still fail with their original error and line context, since the wrapped form is also malformed. Verified: a two-element fragment now passes all layers; `test/wrong/malformed.zul` still fails correctly.

**B4. Dependency bootstrap is fragile outside this machine. ✅ FIXED (2026-07-07).**
...
*Fix applied:* added the PEP 723 block (`requires-python = ">=3.10"`, `dependencies = ["lxml"]`) at the top of `validate-zul.py`; Step 3 now documents `uv run …/validate-zul.py` as the **primary** invocation (uv provisions `lxml` in an ephemeral env — no venv/PEP-668 issues), with `python3` (self-installing `lxml` via `ensure_lxml()`) as the documented fallback and a "on Windows use `python`" note. Verified: `uv run` passes all four layers with no pre-installed lxml.
